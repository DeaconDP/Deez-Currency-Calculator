...
$exe = Join-Path $repoRoot "src-tauri\target\release\deez-currency-calculator.exe"
$logPath = Join-Path $env:TEMP "deez-currency-calculator-launch.log"
...
function Get-ReleaseProcesses {
  ...
  Get-CimInstance Win32_Process -Filter "Name = 'deez-currency-calculator.exe'" -ErrorAction SilentlyContinue |
    Where-Object {
...
function Install-PnpmDependencies {
  ...
  Require-Command "pnpm" "pnpm is required. Install with: npm i -g pnpm"
  Write-Host "Installing dependencies..."
  Write-LaunchLog "pnpm install started"
  pnpm install
  if ($LASTEXITCODE -ne 0) {
    Write-Host "pnpm install failed."
    Write-LaunchLog "pnpm install failed with exit code $LASTEXITCODE"
    exit 1
  }
  Write-LaunchLog "pnpm install succeeded"
}
...
function Build-ReleaseExe {
  ...
  Write-Host "Building Deez Currency Calculator release EXE..."
...
function Start-ReleaseExe {
  ...
  if ($existing.Count -gt 0) {
    Write-Host "Deez Currency Calculator is already running from the release EXE."
    Write-LaunchLog "already running: pid(s) $($existing.ProcessId -join ', ')"
    return $true
  }

  Write-Host "Launching Deez Currency Calculator..."
