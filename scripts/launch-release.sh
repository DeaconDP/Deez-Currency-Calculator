...
APP="$ROOT/src-tauri/target/release/bundle/macos/Deez Currency Calculator.app"
BIN="$ROOT/src-tauri/target/release/deez-currency-calculator"
APP_BIN="$APP/Contents/MacOS/deez-currency-calculator"
LOG="${TMPDIR:-/tmp}/deez-currency-calculator-launch.log"
...
install_pnpm_dependencies() {
  ...
  require_cmd pnpm "pnpm is required. Install with: npm i -g pnpm"
  notify "Installing dependencies…" 18
  log "pnpm install started"
        ...
        for name in filenames:
            ...
            for line in text.splitlines():
                ...
                if not value.startswith("/"):
                    continue
                if "Deez-Currency-Calculator" not in value and "src-tauri/target" not in value:
                    continue
...
build_release() {
  ...
  if [ "$rust_major" -lt 1 ] || { [ "$rust_major" -eq 1 ] && [ "$rust_minor" -lt 77 ]; }; then
    die "Rust 1.77 or newer is required (found $rust_version). Run: rustup update stable"
  fi
...
start_release() {
  ...
  notify "Launching Deez Currency Calculator…" 95
  if [ -d "$APP" ]; then
    log "open app: $APP"
    open "$APP"
  elif [ -x "$BIN" ]; then
