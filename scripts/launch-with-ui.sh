...
STATUS_FILE="${TMPDIR:-/tmp}/deez-currency-calculator-launch-status.json"
LOG="${TMPDIR:-/tmp}/deez-currency-calculator-launch.log"
UI_PORT="${LAUNCH_UI_PORT:-5191}"
...
cleanup() {
  ...
  rm -rf "${TMPDIR:-/tmp}/deez-currency-loading-profile" 2>/dev/null || true
...
on run argv
  ...
  display dialog msg buttons {"Close"} default button "Close" with icon stop with title "Deez Currency Calculator"
end run
APPLESCRIPT
}
...
open_loading_window() {
  local url="$1"
  local profile="${TMPDIR:-/tmp}/deez-currency-loading-profile"
  mkdir -p "$profile"
...
if [ "$status" -eq 0 ]; then
  write_status "Launching Deez Currency Calculator…" 100 true false
  sleep 0.45
  cleanup
  close_terminal
  exit 0
...
error_text="Deez Currency Calculator failed to launch."
if [ -f "$LOG" ]; then
